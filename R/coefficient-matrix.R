...
#'
#' This function processes the RdRp files, merges them, filters the data, and calculates the Spearman correlation.
#' @param input_file The input file prefix.
#' @param output_dir The directory to save the output files.
#' @param threads The number of CPU cores to use for parallel processing.
#' @export
...
calculate_spearman_correlation <- function(input_file, output_dir, threads = 20) {
  ...
  # Read and process the RdRp files
  ...
  data_res <- fread(paste0(input_file, ".megahit.list"), sep = "\t", data.table = F, header = F)
  ...
  list <- fread(paste0(input_file, ".rdrp.list.tsv"), sep = "\t", data.table = F, header = F)
  ...
  fwrite(finalData, file = paste0(input_file, "_RSEM.csv"), quote = FALSE)
  ...
  cor <- cor_construct(data_t, rdrp_contigs, threads)
